...
* miningMode:string - "pplns" or "pps"
...
* [optional] miningMode:string (default="pplns") - mining mode, "pplns" or "pps"
* [optional] totp:string - used only when 2fa for current user activated

### return values:
* status:string - can be one of common status values or:
  * unknown_id: invalid session id
  * invalid_mining_mode: miningMode value is not "pplns" or "pps"
  * pps_not_available: PPS mode is not enabled for this coin

### curl example:
```
curl -X POST -d '{"id": "...session...", "coin": "XPM", "address": "ATWDYBwVDvswyZADMbEo5yBt4tH2zfGjd1", "payoutThreshold": "100", "autoPayoutEnabled": true}' http://localhost:18880/api/userUpdateSettings
```
### response examples:
```
{"status": "ok"}
...
## backendGetPPSConfig
Returns PPS configuration for specified coin. Access: admin and observer only.
...
* status:string - can be one of common status values or:
  * unknown_id: invalid session id or insufficient permissions
  * invalid_coin: coin does not exist
* ppsModeEnabled:boolean - whether PPS mode is enabled
* ppsPoolFee:double - pool fee percentage
* ppsSaturationFunction:string - saturation function name ("none", "tanh", "clamp", "cubic", "softsign", "norm", "atan", "exp")
* ppsSaturationB0:double - saturation balance scale parameter
* ppsSaturationANegative:double - saturation amplitude for negative balance
* ppsSaturationAPositive:double - saturation amplitude for positive balance
* saturationFunctions:[string] - list of all supported saturation function names

### curl example:
```
curl -X POST -d '{"id": "...session...", "coin": "BTC"}' http://localhost:18880/api/backendGetPPSConfig
```
...
## backendUpdatePPSConfig
Update PPS configuration for specified coin. Access: admin only.

### arguments:
* [required] id:string - admin session id
* [required] coin:string
* [required] ppsModeEnabled:boolean - enable or disable PPS mode
* [required] ppsPoolFee:double - pool fee percentage
* [optional] ppsSaturationFunction:string (default="") - saturation function name, empty string means "none"
* [optional] ppsSaturationB0:double (default=0.0) - saturation balance scale; required and must be > 0 when saturation function is not "none"
* [optional] ppsSaturationANegative:double (default=0.0) - saturation amplitude for negative balance; must be in [0, 1]
* [optional] ppsSaturationAPositive:double (default=0.0) - saturation amplitude for positive balance; must be in [0, 1]
...
### curl example:
```
curl -X POST -d '{"id": "...session...", "coin": "BTC", "ppsModeEnabled": true, "ppsPoolFee": 4.0, "ppsSaturationFunction": "tanh", "ppsSaturationB0": 3.0, "ppsSaturationANegative": 0.5, "ppsSaturationAPositive": 0.3}' http://localhost:18880/api/backendUpdatePPSConfig
```
