...
// ------------------------------------------------------------------------------
	...
	*/
	...
	static void write_channel_header(FILE* file, Verbosity verbosity)
	{
		if (!s_arguments.empty()) {
			fprintf(file, "arguments: %s\n", s_arguments.c_str());
		}
	...
	static bool expand_strftime_path(const char* pattern, char* out, size_t out_size)
	{
		auto t = time(nullptr);
		struct tm time_info;
		localtime_r(&t, &time_info);
		return strftime(out, out_size, pattern, &time_info) > 0;
	}

	bool LogChannel::open(const char* path_in, FileMode mode, Verbosity verbosity)
	{
		close();
		...
		snprintf(pathPattern_, sizeof(pathPattern_), "%s", path);
		mode_ = mode;
		verbosity_ = verbosity;

		if (!expand_strftime_path(pathPattern_, currentPath_, sizeof(currentPath_))) {
			snprintf(currentPath_, sizeof(currentPath_), "%s", pathPattern_);
		}

		if (!create_directories(currentPath_)) {
			LOG_F(ERROR, "LogChannel: failed to create directories for '" LOGURU_FMT(s) "'", currentPath_);
		}

		const char* mode_str = (mode == FileMode::Truncate ? "w" : "a");
#ifdef _WIN32
		file_ = _fsopen(currentPath_, mode_str, _SH_DENYNO);
#else
		file_ = fopen(currentPath_, mode_str);
#endif
		if (!file_) {
			LOG_F(ERROR, "LogChannel: failed to open '" LOGURU_FMT(s) "'", currentPath_);
			return false;
		}
	...
	bool LogChannel::rotateIfNeeded()
	{
		char newPath[PATH_MAX];
		if (!expand_strftime_path(pathPattern_, newPath, sizeof(newPath))) {
			return false;
		}

		if (strcmp(newPath, currentPath_) == 0) {
			return false;
		}
		...
		snprintf(currentPath_, sizeof(currentPath_), "%s", newPath);

		if (!create_directories(currentPath_)) {
			LOG_F(ERROR, "LogChannel: failed to create directories for '" LOGURU_FMT(s) "'", currentPath_);
		}

		const char* mode_str = (mode_ == FileMode::Truncate ? "w" : "a");
#ifdef _WIN32
		file_ = _fsopen(currentPath_, mode_str, _SH_DENYNO);
#else
		file_ = fopen(currentPath_, mode_str);
#endif
		if (!file_) {
			LOG_F(ERROR, "LogChannel: failed to open '" LOGURU_FMT(s) "' after rotation", currentPath_);
			return false;
		}
		...
		write_channel_header(file_, verbosity_);

		VLOG_F(g_internal_verbosity, "LogChannel: rotated to '" LOGURU_FMT(s) "'", currentPath_);
		return true;
	}

	void log_to_channel(LogChannel* channel, Verbosity verbosity,
	                     const char* file, unsigned line, const char* formatted_message)
	{
		char preamble_buff[LOGURU_PREAMBLE_WIDTH];
		print_preamble(preamble_buff, sizeof(preamble_buff), verbosity, file, line);
		...
		// Write to channel file
		{
			...
			channel->rotateIfNeeded();

			if (channel->file_) {
				fprintf(channel->file_, "%s%s\n", preamble_buff, formatted_message);
				if (g_flush_interval_ms == 0) {
		...
		// Duplicate to stderr (with colors, same as log_message)
		if (verbosity <= g_stderr_verbosity) {
			std::lock_guard<std::recursive_mutex> lock(s_mutex);
			if (g_colorlogtostderr && s_terminal_has_color) {
				if (verbosity > Verbosity_WARNING) {
