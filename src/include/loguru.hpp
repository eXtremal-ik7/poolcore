...
#undef MAX
	...
	*/
...
#if __cplusplus >= 202002L
	// Log channel — writes to a separate file, independent of global loguru output.
	// path can be a strftime pattern (e.g. "logs/backend-%Y-%m-%d.log") for automatic rotation.
	// If path contains no % specifiers, no rotation is performed.
	class LOGURU_EXPORT LogChannel {
	public:
		LogChannel() = default;
		~LogChannel();
		...
		bool open(const char* path, FileMode mode, Verbosity verbosity);
		void close();
		...
		void log(Verbosity verbosity, const char* file, unsigned line,
			...
			log_to_channel(this, verbosity, file, line, msg.c_str());
		}

	private:
		FILE* file_ = nullptr;
		Verbosity verbosity_ = Verbosity_OFF;
		FileMode mode_ = Append;
		char pathPattern_[PATH_MAX] = {};
	...
	// Global log channel: fallback when no thread-local channel is set.
	LOGURU_EXPORT extern LogChannel* g_channelLog;
	LOGURU_EXPORT void set_global_channel_log(LogChannel* ch);

	// Thread-local log channel: set once per thread, used by CLOG_F macro.
	inline thread_local LogChannel* t_channelLog = nullptr;
	inline void set_channel_log(LogChannel* ch) { t_channelLog = ch; }
#endif
...
// CLOG_FC(channel, INFO, "Block found: height={}", height);
// Logs to a specific LogChannel using std::format syntax.
#if __cplusplus >= 202002L
#define CLOG_FC(channel, verbosity_name, ...)                                                      \
	(channel).log(loguru::Verbosity_ ## verbosity_name, __FILE__, __LINE__, __VA_ARGS__)

// CLOG_F(INFO, "Block found: height={}", height);
// Uses thread-local LogChannel set via loguru::set_channel_log().
#define CLOG_F(verbosity_name, ...)                                                                \
	do {                                                                                           \
		if (auto *_ch_ = loguru::t_channelLog ? loguru::t_channelLog : loguru::g_channelLog)       \
			_ch_->log(loguru::Verbosity_ ## verbosity_name, __FILE__, __LINE__, __VA_ARGS__);      \
	} while(0)
#endif
