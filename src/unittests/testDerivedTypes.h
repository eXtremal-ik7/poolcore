...
inline void __testValSerialize(std::string &out, int64_t val, uint32_t ctx)
{
  int64_t scale = 1;
  for (uint32_t i = 0; i < ctx; i++)
    scale *= 10;
...
inline void __aValSerialize(std::string &out, int64_t val, uint32_t ctx)
{
...
inline void __bValSerialize(std::string &out, int64_t val, uint32_t ctx)
{
  __testValSerialize(out, val, ctx);
}

// scaledNumber: int64_t from JSON number literal with scale factor (context = decimal places)
inline bool __scaledNumberResolve(const std::string &raw, uint32_t ctx, int64_t &out)
{
  return __testValResolve(raw, ctx, out);
}

inline void __scaledNumberSerialize(std::string &out, int64_t val, uint32_t ctx)
{
  // Serialize as bare JSON number (no quotes)
  int64_t scale = 1;
  for (uint32_t i = 0; i < ctx; i++)
    scale *= 10;
  if (ctx == 0) {
  ...
  while (fracStr.size() < ctx)
    fracStr = "0" + fracStr;
  out.append(std::to_string(whole) + "." + fracStr);
}
...
inline void __ctxCounter32ValSerialize(std::string &out, uint32_t val, uint32_t ctx)
{
